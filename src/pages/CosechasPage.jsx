...
const CosechasPage = () => {
  ...
  const renderTablaCultivo = (cultivo) => {
    ...
    const rows = items.map((item) => {
      ...
      const camDelLote = camiones.filter((c) => c.cosecha_id === item.id);
      const kgCampo = camDelLote.reduce((a, c) => a + (c.kg_campo || 0), 0);
      const kgCec = camDelLote
        .filter((c) => c.camion_para === "Cecilia")
        .reduce((a, c) => a + (c.kg_campo || 0), 0);
      const kgHor = camDelLote
    ...
    return (
      <div className="rounded-xl border bg-white shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2 bg-[#f1f4f3] border-b">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows}
            <tr className="font-semibold border-t bg-[#f1f4f3]">
    ...
    <div className="w-full min-h-screen pb-12 px-2">
      ...
      {campaniaSeleccionada && cultivoSeleccionado && (
        <div className="flex flex-col gap-8">
          {/* Fila superior: entregas + balance */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            {/* Entregado por destino */}
            <div className="rounded-xl border bg-white shadow-sm overflow-hidden">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  <tr className="font-semibold border-t bg-[#f1f4f3]">
                    ...
                    <td className="px-4 py-2 text-right">
                      {formatNumber(resumenPorDestino.reduce((a, r) => a + r.cecilia, 0))}
                    </td>
                    <td className="px-4 py-2 text-right">
                      {formatNumber(resumenPorDestino.reduce((a, r) => a + r.horacio, 0))}
                    </td>
                    <td className="px-4 py-2 text-right">
                      {formatNumber(
                        resumenPorDestino.reduce((a, r) => a + r.cecilia + r.horacio, 0)
                      )}
                    </td>
                  </tr>
                </tbody>
            ...
            {(() => {
              const cosechasHoracio = (cosechasAgrupadas[cultivoSeleccionado] || []).filter(
                (c) => {
                  const si = siembras.find(
                    (s) => s.campania === c.campania && s.lote === c.lote
                  );
                  return si?.productor === "Horacio";
                }
              );
              if (!cosechasHoracio.length) return null;

              const camHor = camiones.filter((c) =>
                cosechasHoracio.some((cs) => cs.id === c.cosecha_id)
              );

              const kgCampo = camHor.reduce((sum, c) => sum + (c.kg_campo || 0), 0);
              const kgHor = camHor
                .filter((c) => c.camion_para === "Horacio")
              ...
              const kgCec = camHor
                ...
                .reduce((sum, c) => sum + (c.kg_campo || 0), 0);
              const pctCec = kgCampo ? ((kgCec / kgCampo) * 100).toFixed(1) : "-";
              const idealPct = cultivoSeleccionado === "Soja" ? 33.5 : 27;
              const diffKg = Math.round((idealPct / 100) * kgCampo - kgCec);
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      <tr className="border-t">
                        <td className="px-4 py-2">{formatNumber(kgCampo)}</td>
                        <td className="px-4 py-2">{formatNumber(kgHor)}</td>
                        <td className="px-4 py-2">{formatNumber(kgCec)}</td>
          ...
          {/* Fila inferior: Resultado lotes + Camiones Lote X */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
              ...
              <div className="overflow-y-auto max-h-[calc(100vh-200px)]">
                {loteSeleccionado && camionesFiltrados.length > 0 ? (
                  ...
                  <p className="text-sm text-gray-500 p-4">
                    {loteSeleccionado ? "No hay camiones registrados." : "Seleccionar un lote"}
                  </p>
                )}
              </div>
            </div>
